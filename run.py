...
class DANN:
  ...
  def train(self,n_epoch,batch_size,train,test,folder):
    ...
    for epoch in range(n_epoch):
      ...
      while((index_batch)*batch_size < len(s_label)):
        ...
        writer1.add_scalar('err_s_label', err_s_label, epoch+folder*100)
        writer1.add_scalar('err_s_domain', err_s_domain, epoch+folder*100)
        acc_s_label = self.acc_rate(class_output,s_label_mini)
        acc_s_domain = self.acc_rate(domain_output,domain_label_mini)
        writer1.add_scalar('acc_s_label', acc_s_label, epoch+folder*100)
        writer1.add_scalar('acc_s_domain', acc_s_domain, epoch+folder*100)
        if ((index_batch+1)*batch_size < len(s_label)):
        ...
        acc_t_domain = self.acc_rate(domain_output,domain_t_label_mini)
        writer1.add_scalar('err_t_domain', err_t_domain, epoch+folder*100)
        writer1.add_scalar('acc_t_domain', acc_t_domain, epoch+folder*100)

        err = 0.1*err_t_domain + err_s_domain + err_s_label
        err.backward()
        self.optimizer.step()
        ...
        # writer1.add_scalar('acc_t_domain', err_t_domain, epoch+folder*100)
  ...
  def run(self):
    for folder in range(domain_num):
      ...
      self.train(100,400,train,test, folder)
      self.test(test)
      # self.test_init(test)
